...
export function App() {
  ...
  const saveMemoryCardImage = async () => {
    const card = cardRef.current;
    if (!card) return;
    try {
      const rect = card.getBoundingClientRect();
      const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="${Math.ceil(rect.width*2)}" height="${Math.ceil(rect.height*2)}">
        <foreignObject width="100%" height="100%" transform="scale(2)">${new XMLSerializer().serializeToString(card)}</foreignObject>
      </svg>`;
      ...
      link.download = `어법-암기카드-CH${currentChapter.id}.png`;
      link.href = canvas.toDataURL('image/png');
      link.click();
      setSaveStatus(`CH ${currentChapter.id} ${currentChapter.title} 카드 이미지를 저장했습니다.`);
    } catch {
      setSaveStatus('이미지 저장에 실패했습니다. 브라우저 권한 또는 환경을 확인한 뒤 다시 시도해 주세요.');
    }
  };
...
{activeTab==='암기' && <div className="space-y-3 text-sm">
  <div ref={cardRef} className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
    <p className="text-xs font-semibold text-slate-500">저장될 암기카드 미리보기</p>
    <h4 className="mt-1 text-lg font-semibold">CH {currentChapter.id}. {currentChapter.title}</h4>
    <p className="mt-2">핵심 직관: {currentChapter.intuition}</p>
    <p className="mt-2">판단 알고리즘 요약: {currentChapter.examAlgorithm.slice(0,3).join(' → ')}</p>
    <p className="mt-2">대표 함정: {currentChapter.commonTraps.slice(0,2).map((t)=>t.title).join(', ')}</p>
    <p className="mt-2">예문: {currentChapter.examples[0]?.sentence}</p>
    <p className="mt-2">암기 포인트: {currentChapter.mustRemember[0]}</p>
  </div>
  <button className="btn-primary" onClick={saveMemoryCardImage}>암기카드 이미지 저장</button>
  {saveStatus && <p className="text-xs text-slate-600">{saveStatus}</p>}
  {currentChapter.mustRemember.map((m,idx)=><div key={idx} className="rounded-lg border border-amber-200 bg-amber-50 p-2">• {m}</div>)}
</div>}</div></div></div></PremiumCard>}
        ...
        {(desktopSection === 'lab' || mobileTab === 'lab') && <PremiumCard><SectionHeader title="문장 해부 실험실" description="코드 기반 구조 시각화로 주어·동사·수식어를 한눈에 분해합니다."/><p className="mt-3 text-sm text-slate-600">총 {sentenceBank.length}문장 데이터 학습 가능</p>
<div className="mt-3 grid gap-2">
  <div className="flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-indigo-100 px-3 py-1">주어</span><span className="rounded-full bg-emerald-100 px-3 py-1">동사</span><span className="rounded-full bg-amber-100 px-3 py-1">목적어/보어</span><span className="rounded-full bg-slate-200 px-3 py-1">수식어</span></div>
  <div className="rounded-xl border border-stone-200 bg-white p-3 text-sm break-words">The <span className="rounded bg-indigo-100 px-1">quality</span> <span className="rounded bg-slate-200 px-1">of the students</span> <span className="rounded bg-emerald-100 px-1">is improving</span>.</div>
  <p className="text-xs text-slate-600">학습 흐름: 뼈대 찾기 → 연결 장치 확인 → 함정 배제 → 정답 근거 말하기</p>
</div></PremiumCard>}
      </section>
    </main>
